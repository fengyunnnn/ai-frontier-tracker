...
### 14.6 已落地：总览重构为四层全景（2026-09-14）

负责人指出 14.5 的总览仍有两个问题：**范围**上仍以运营商准入为主轴，而网页实际追踪的是「语音交互与 AI 人机交互」全景（技术前沿／产品动态／竞品格局／政策标准／内部能力五条线并存）；**结构**上四个 Level 是分层判断，适合作决策框架，但不适合作为总览的全部。要求按「能力层—产品层—产业层—我们自己」四层重写，并在每层末尾标注对应章节。

**落地内容**

- 开头新增一句话总判断：主线正从「把语音接进设备」转向「让助手在真实场景里把事办成」，并点明转变同时发生在能力、产品、产业、内部准备四个层面；
...
- **【我们自己】**（对应八、公司内部进展）：保留 Level 2／Level 3／横向诊断轴，并补落点——不追逐前沿能力，而是把已有语音积累转为终端体验长板，补齐家庭入口的「身份—设备—任务」三层平台能力。

**契约保护（关键）**

`sync-content.mjs` 的 `directions` 只识别总览内 `^数字.` 行（编号 ≤ 4），并取**紧邻下一行**为卡片说明；页面「竞争判断分层」卡片（`report.directions`）的唯一数据源就是这 4 条。因此**四个编号条目逐字保留**，只在其前后增加四层叙述 —— 框架是**扩展而非替换**：

| 编号条目 | 归属于 |
|---|---|
| `1. Level 1｜生存层：准入与内容闭环` | 产业层 |
| `2. Level 2｜竞争层：自然交互与真实场景` | 我们自己 |
| `3. Level 3｜未来层：任务编排与跨端智能` | 我们自己 |
| `4. 横向诊断轴｜先归因，再投入` | 我们自己 |

**与负责人草稿的一处偏离（待确认）**：草稿把 Level 2／3 的归属同时写成「产品层」与「我们自己」，两处表述不一致。本次按「具体改动」两条的明确指派落地 —— Level 1 入产业层，Level 2／3 与横向诊断轴入我们自己。若希望 Level 2 改归产品层，只需移动一行，不影响任何契约。

**新增契约与回归测试**

- `docs/CONTENT_SCHEMA.md` §3 新增「总览结构」小节，写死四层顺序、每层一个 `→ 对应章节：` 标记、四个编号条目必须保留且编号 1—4 连续、每条编号行之后紧邻一行说明；
- `tests/governance.test.mjs` 新增第 7 个测试 `总览 covers capability, product, industry and internal layers`，断言四层齐全有序、`→ 对应章节：` 标记数为 4、四个编号条目及其紧邻说明行均存在。
...
| 无缺陷基线 | 通过 |
| 删除 `【能力层】` 标题行 | 测试失败（拦住）|
| 删除一个 `→ 对应章节：` 标记 | 测试失败（拦住）|
| 删除 `1. Level 1｜…` 判断条目 | 测试失败（拦住）|
| 还原后 | 逐字节一致并通过 |

`REGRESSIONS NOT CAUGHT: none`。**该测试首次运行即抓到一个真实疏漏**：产业层的 `→ 对应章节：四、行业动态` 标记被漏写（标记数为 3 而非 4），已补齐。这直接证明断言不是空断言。

**验证**：门禁 `sync-content`（8 sections / 12 highlights）→ `node --test` **7/7** → `lint` → `tsc --noEmit` → `vite build` 全过（750.43 kB，属已知非阻断大包警告）。浏览器实测（Edge + CDP，`http://127.0.0.1:4173/`）：四层齐全且顺序正确、`→ 对应章节：` 4 个、判断卡片 4 张（01—04 标题无误）、导航 8 章齐全、一句话判断已渲染。
