...
**新增可复用探针（`_wb_recovery_20260914/scripts/`）**：`_wb_cdp_nav.mjs`（读导航标签与激活项）、`_wb_cdp_nav2.mjs`（多视口裁剪测量）、`_wb_cdp_content.mjs`（章节正文与二级导航）、`_wb_cdp_order.mjs`（展开全部模式章节顺序）。均通过 Node 内置 `WebSocket` 直连 Edge CDP，**无需安装 Playwright/Puppeteer**。
